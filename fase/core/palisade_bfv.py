...
class PalisadeBFVContext():
    """All inputs to Palisade BFV scheme are expected to be lists of integers. 
    ...
    """
    def __init__(self, plaintext_modulus, numAdds, numMults, numKeyswitches,
                 sigma=3.2, rot=None, silent=False):
        self._rot_indices = []
        self._silent = silent
        self._scheme = pycrypto.BFVwrapper()
        self._scheme.KeyGen(plaintext_modulus,
                            sigma, 
                            numAdds,
                            numMults, 
                            numKeyswitches)
